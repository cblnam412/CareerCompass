...
const questionKeys = ['question', 'Question', 'Cau hoi', 'Cau hỏi', 'Noi dung', 'Nội dung', 'question_text'];
...
export const parseExcelQuestions = (buffer) => {
  if (!buffer) throw new HttpError(400, 'Vui long upload file Excel');

  const workbook = xlsx.read(buffer, { type: 'buffer' });
  const sheet = workbook.Sheets[workbook.SheetNames[0]];
  if (!sheet) throw new HttpError(400, 'File Excel khong co sheet hop le');
...
export const validateQuestionsStructure = (questions) => {
  ...
  if (!Array.isArray(questions) || questions.length === 0) {
    return { valid: false, errors: ['Questions phai la mang va khong duoc rong'] };
  }
  ...
  questions.forEach((question, index) => {
    ...
    if (!question.question || typeof question.question !== 'string' || question.question.trim() === '') {
      errors.push(`${label}: Noi dung cau hoi khong hop le`);
    }

    if (!Array.isArray(question.options) || question.options.length !== 4) {
      errors.push(`${label}: Can dung 4 lua chon`);
    } else {
      question.options.forEach((option, optionIndex) => {
        if (!option || typeof option !== 'string' || option.trim() === '') {
          errors.push(`${label}, Option ${optionIndex + 1}: Lua chon khong duoc rong`);
        }
      });
    }

    if (!question.answer || typeof question.answer !== 'string' || question.answer.trim() === '') {
      errors.push(`${label}: Dap an khong hop le`);
    } else if (Array.isArray(question.options) && !question.options.includes(question.answer)) {
      errors.push(`${label}: Dap an khong nam trong cac lua chon`);
    }
  });
