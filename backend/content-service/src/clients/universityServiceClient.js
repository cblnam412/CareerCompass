...
export const getMajorById = (majorId) =>
  requestUniversityService(`/api/universities/majors/id/${majorId}`);

export const getMajorsByIds = async (majorIds = []) => {
  const uniqueIds = [...new Set(majorIds.map(String).filter(Boolean))];
  const results = await Promise.allSettled(uniqueIds.map((id) => getMajorById(id)));
  return results.map((result, index) =>
    result.status === 'fulfilled'
      ? result.value
      : { _id: uniqueIds[index], lookupError: result.reason?.message || 'Lookup failed' },
